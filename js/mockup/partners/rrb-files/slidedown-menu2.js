...
function setStyle1() {
	...
	for(var no=0;no<aTags.length;no++){
		...
		if (currentHref == aTags[no].href) {
			document.getElementById(aTags[no].id).style.color='#DAA520';
		}
	}
}

function setStyle2(link_id) {
	var li_id = 'i' + link_id;
	var a_id = 'a' + link_id;
	document.getElementById(a_id).style.color='#DAA520';
	document.getElementById(li_id).setAttribute("id","initialExpandedMenuItem");
}
...
function showSubMenu(e,inputObj)
{
	...
	if(e || inputObj) {
		...
		if(dg_activeItem) {
			dg_activeItem.className = dg_activeItem.className.replace('activeItem','');
		}
...
function scrollDownSub()
{
	if(divToScroll){
		...
		var offsetMove =Math.min(60,(ulToScroll.offsetHeight - height));
		...
		if(height<ulToScroll.offsetHeight)setTimeout('scrollDownSub()',40); else {
			divToScroll = false;
			ulToScroll = false;
